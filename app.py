...
# =============================================================================
# GOOGLE SHEETS MANAGER - ENHANCED FOR MULTIPLE DATASETS
# =============================================================================
            ...
            if st.button("Créer le compte", type="primary", use_container_width=True):
                if new_user and new_pass:
                    if new_pass != confirm_pass:
                        ...
                        if st.session_state.gsheets.create_user(new_user, new_pass, email, is_admin):
...
def dataset_management_page():
    ...
    with tab3:
        if not st.session_state.is_admin:
            st.warning("⚠️ Cette section est réservée aux administrateurs")
            return
...
def annotate_page():
    ...
    with col1:
        if st.button("Exporter mes annotations", use_container_width=True):
            export_data = gsheets.export_user_annotations(username, selected_id)
            if export_data:
                json_str = json.dumps(export_data, indent=2, ensure_ascii=False)
                st.download_button(
                    "⬇️ Mes annotations",
    ...
    with col2:
        if st.button("Exporter dataset complet", use_container_width=True):
            export_data = gsheets.export_annotated_dataset(selected_id)
            if export_data:
                json_str = json.dumps(export_data, indent=2, ensure_ascii=False)
                st.download_button(
                    "⬇️ Dataset complet",
                    json_str,
                    file_name=f"causafr_full_{selected_id}_{datetime.now().strftime('%Y%m%d')}.json",
                    mime="application/json",
                    use_container_width=True
                )
    
    with col3:
        if st.button("Exporter tous", use_container_width=True):
            all_datasets = gsheets.get_datasets()
            all_export_data = []
            
            for ds in all_datasets:
                export_data = gsheets.export_annotated_dataset(ds.dataset_id)
                if export_data:
            ...
            if all_export_data:
                combined_data = {
                    'metadata': {
                        'exported_at': datetime.now().isoformat(),
                        'total_datasets': len(all_export_data),
                        'format': 'combined_datasets'
                    },
                    'datasets': all_export_data
                }
                ...
                st.download_button(
                    "⬇️ Tous les datasets",
                    json_str,
                    file_name=f"causafr_all_datasets_{datetime.now().strftime('%Y%m%d')}.json",
                    mime="application/json",
                    use_container_width=True
                )
...
def about_page():
    ...
    <div class="card">
        ...
        <h3>🔧 Architecture Google Sheets</h3>
        <p><strong>5 feuilles structurées :</strong></p>
        <ol>
            <li><strong>users</strong> - Gestion des comptes utilisateurs</li>
            <li><strong>datasets</strong> - Métadonnées des datasets (nom, description, statistiques)</li>
            <li><strong>dataset_pairs</strong> - <em>28 colonnes</em> pour stocker TOUS les champs des JSON originaux</li>
            <li><strong>annotations</strong> - Annotations des utilisateurs avec métadonnées</li>
            <li><strong>progress</strong> - Suivi de progression par utilisateur par dataset</li>
        </ol>
        
        <h3>🚀 Fonctionnalités clés</h3>
        <ul>
            <li>✅ <strong>Multi-datasets</strong> - Importez plusieurs fichiers JSON</li>
            <li>✅ <strong>Préservation complète</strong> - Tous les champs JSON originaux conservés</li>
            <li>✅ <strong>Export riche</strong> - JSON avec données originales + annotations</li>
            <li>✅ <strong>Pas de fichiers locaux</strong> - Tout dans Google Sheets</li>
            <li>✅ <strong>Interface riche</strong> - Affichage des marqueurs causaux/temporels</li>
            <li>✅ <strong>Gestion admin</strong> - Suppression de datasets, statistiques avancées</li>
        </ul>
        
        <h3>📊 Formats d'export</h3>
        <p>Trois options d'export :</p>
        <ol>
            <li><strong>Vos annotations seulement</strong> - Vos décisions pour un dataset</li>
            <li><strong>Dataset complet annoté</strong> - Toutes les annotations + données originales</li>
            <li><strong>Tous les datasets</strong> - Export combiné de tous les datasets</li>
        </ol>
