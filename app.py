...
def load_datasets_from_json():
    """Charge les datasets depuis les fichiers JSON"""
    datasets = []
    data_dir = "data"
            ...
            try:
                with open(os.path.join(data_dir, filename), 'r', encoding='utf-8') as f:
                    ...
                    datasets.append({
                        ...
                        'filename': filename,
                        'data': data,
                        'pairs_count': len(data.get('pairs', [])) if isinstance(data, dict) else 0
                    })
            except Exception as e:
                st.error(f"Erreur chargement {filename}: {str(e)}")
...
class Annotation:
    ...
    cue1: bool
    cue2: bool
    label: Optional[int]  # 0, 1 ou None
    confidence: int  # 1-5
    notes: str
...
class GoogleSheetsManager:
    ...
    def _connect(self):
        ...
        try:
            # Configuration via secrets Streamlit
...
def annotate_page():
    ...
    with st.container(border=True):
        ...
        with col_pair1:
            ...
            # Cues et temporalités originales
    ...
    # Vérifier s'il y a déjà des annotations pour cette paire
    ...
    if existing_annotations:
        ...
        for ann in existing_annotations[:3]:  # Montrer les 3 premières
            with st.container(border=True):
                ...
                with col_ann2:
                    if st.button("Réutiliser", key=f"reuse_{ann.annotated_at}"):
                        # Pré-remplir le formulaire avec cette annotation
    ...
    with st.form(key="annotation_form", border=True):
        ...
        with col_form1:
            ...
            label = st.radio(
                ...
                horizontal=True,
                index=2 if not existing_annotations else 
                      (0 if existing_annotations[-1].label == 1 else 
                       (1 if existing_annotations[-1].label == 0 else 2))
            )
        ...
        with col_form2:
            ...
            cue1 = st.checkbox(
                ...
                value=pair.get('event1_has_causal_cue', False) if not existing_annotations 
                       else existing_annotations[-1].cue1,
                help="L'événement 1 contient-il un mot/phrase indiquant une causalité?"
            )
            
            cue2 = st.checkbox(
                f"Cue dans Événement 2",
                value=pair.get('event2_has_causal_cue', False) if not existing_annotations 
                       else existing_annotations[-1].cue2,
...
def export_page():
    """Page d'export avec gestion complète des cues"""
    st.title("📤 Export des annotations")
    
    # Vérifier la connexion
    if not gsheets.connected:
        st.warning("⚠️ Mode local - Les données seront sauvegardées en session seulement")
    
    # Section de configuration
    with st.expander("⚙️ Configuration de l'export", expanded=True):
        col1, col2 = st.columns(2)
        ...
        with col1:
            ...
            export_format = st.selectbox(
                "Format de sortie",
                ["json", "csv", "excel"],
                help="JSON conserve la structure originale, CSV/Excel pour analyse"
            )
            
            # Stratégie de fusion
            merge_strategy = st.selectbox(
                "Fusion multi-annotations",
                ["latest", "majority", "all"],
                format_func=lambda x: {
                    "latest": "Dernière annotation",
                    "majority": "Majorité (vote)",
                    "all": "Toutes les annotations"
                }[x],
                help="Comment fusionner quand plusieurs annotations existent"
            )
        
        with col2:
            # Champs à inclure
            st.markdown("**Champs à inclure:**")
            col_chk1, col_chk2 = st.columns(2)
            with col_chk1:
                include_labels = st.checkbox("Labels", value=True)
                include_cues = st.checkbox("Cues", value=True)
                include_confidence = st.checkbox("Confiance", value=True)
            with col_chk2:
                include_notes = st.checkbox("Notes", value=False)
                include_conflicts = st.checkbox("Conflits", value=False)
                include_metadata = st.checkbox("Métadonnées", value=True)
    ...
    with st.expander("🔍 Filtres", expanded=False):
        ...
        with col_f1:
            ...
            if users:
                ...
                selected_user = st.selectbox("Annotateur", user_options)
            else:
                selected_user = "Tous les utilisateurs"
        
        with col_f2:
            # Filtre par label
            label_options = ["Tous", "Causal (1)", "Non-causal (0)", "Incertain (None)", "Non annoté"]
            selected_label = st.selectbox("Label", label_options)
        
        with col_f3:
            # Filtre par date
            date_range = st.date_input(
                "Période d'annotation",
                value=(datetime.now() - timedelta(days=7), datetime.now()),
                help="Filtrer par date d'annotation"
            )
    ...
    if st.button("🚀 Générer l'export", type="primary", use_container_width=True):
        with st.spinner("🔄 Préparation de l'export..."):
            try:
                ...
                # Récupérer les annotations avec filtres
                all_annotations = gsheets.get_annotations_for_dataset(selected_dataset)
                
                # Appliquer les filtres
                filtered_annotations = all_annotations
                
                if selected_user != "Tous les utilisateurs":
                    filtered_annotations = [a for a in filtered_annotations 
                                          if a.username == selected_user]
                
                if len(date_range) == 2:
                    start_date, end_date = date_range
                    filtered_annotations = [a for a in filtered_annotations 
                                          if start_date <= a.annotated_at.date() <= end_date]
                ...
                if export_format == "json":
                    ...
                    download_json_export(export_data, selected_dataset)
                
                else:  # CSV ou Excel
                    export_df = generate_tabular_export(
                        original_data=original_data,
                        annotations=filtered_annotations,
                        merge_strategy=merge_strategy,
                        include_labels=include_labels,
                    ...
                    if export_format == "csv":
                        download_csv_export(export_df, selected_dataset)
                    else:
                        download_excel_export(export_df, selected_dataset)
                
                # Afficher les statistiques
                display_export_statistics(original_data, filtered_annotations)
...
def generate_json_export(
    ...
    """Génère un export JSON avec mise à jour des champs"""
    ...
    # Organiser les annotations par paire
    annotations_by_pair = {}
    for ann in annotations:
        ...
        annotations_by_pair[key].append(ann_dict)
    ...
    for pair in export_data["pairs"]:
        ...
        # Créer une copie de la paire
        pair_copy = pair.copy()
        
        if key in annotations_by_pair:
            pair_annotations = annotations_by_pair[key]
            
            # Fusionner selon la stratégie
            merged = merge_annotations_data(pair_annotations, merge_strategy)
            
            # Mettre à jour le label
            if include_labels and merged.get("label") is not None:
                pair_copy["label"] = merged["label"]
            
            # Mettre à jour les cues
            if include_cues:
                # Mettre à jour event1_has_causal_cue
                if merged.get("cue1") is not None:
                    pair_copy["event1_has_causal_cue"] = merged["cue1"]
                    # Préserver le texte original si disponible
                    if merged["cue1"] and not pair_copy.get("event1_causal_cue_text"):
                        pair_copy["event1_causal_cue_text"] = "annotated_cue"
                    elif not merged["cue1"]:
                        pair_copy["event1_causal_cue_text"] = ""
                        pair_copy["event1_causal_cue_type"] = ""
                
                # Mettre à jour event2_has_causal_cue
                if merged.get("cue2") is not None:
                    pair_copy["event2_has_causal_cue"] = merged["cue2"]
                    if merged["cue2"] and not pair_copy.get("event2_causal_cue_text"):
                        pair_copy["event2_causal_cue_text"] = "annotated_cue"
                    elif not merged["cue2"]:
                        pair_copy["event2_causal_cue_text"] = ""
                        pair_copy["event2_causal_cue_type"] = ""
            
            # Ajouter la confiance
            if include_confidence and merged.get("confidence") is not None:
                pair_copy["annotation_confidence"] = merged["confidence"]
            
            # Ajouter les notes
            if include_notes and merged.get("notes"):
                pair_copy["annotation_notes"] = merged["notes"]
            
            # Gérer les conflits
            if include_conflicts and len(pair_annotations) > 1:
                pair_copy["has_annotation_conflict"] = has_conflict(pair_annotations)
                if pair_copy["has_annotation_conflict"]:
                    pair_copy["conflict_details"] = get_conflict_details_data(pair_annotations)
        
        # Appliquer le filtre de label
        if not apply_label_filter(pair_copy, label_filter):
            continue
        
        processed_pairs.append(pair_copy)
    
    export_data["pairs"] = processed_pairs
    ...
    if include_metadata:
        export_data["export_metadata"] = {
            ...
            "merge_strategy": merge_strategy,
            "total_pairs_exported": len(processed_pairs),
            "statistics": {
                "pairs_with_annotations": sum(1 for p in processed_pairs 
                                            if (p.get("narrative_id"), 
                                                p.get("event1_id"), 
                                                p.get("event2_id")) in annotations_by_pair),
                "avg_confidence": np.mean([p.get("annotation_confidence", 0) 
                                         for p in processed_pairs 
                                         if "annotation_confidence" in p]) or 0
            }
        }
    
    return export_data

def merge_annotations_data(annotations: List[Dict], strategy: str) -> Dict:
    """Fusionne les annotations selon la stratégie"""
    
    if not annotations:
        return {}
    
    if strategy == "latest":
        # Prendre la dernière annotation
        return max(annotations, key=lambda x: x.get("annotated_at", ""))
    
    elif strategy == "majority":
        # Vote majoritaire
        labels = [a["label"] for a in annotations if a["label"] is not None]
        cues1 = [a["cue1"] for a in annotations if a["cue1"] is not None]
        cues2 = [a["cue2"] for a in annotations if a["cue2"] is not None]
        
        merged = {}
        
        if labels:
            label_counts = Counter(labels)
            merged["label"] = label_counts.most_common(1)[0][0]
        
        if cues1:
            cue1_counts = Counter(cues1)
            merged["cue1"] = cue1_counts.most_common(1)[0][0]
        
        if cues2:
            cue2_counts = Counter(cues2)
            merged["cue2"] = cue2_counts.most_common(1)[0][0]
        
        # Moyenne de confiance
        confidences = [a.get("confidence", 0) for a in annotations]
        merged["confidence"] = np.mean(confidences) if confidences else 0
        
        # Notes: prendre celles de la dernière annotation
        merged["notes"] = annotations[-1].get("notes", "")
        
        return merged
    
    elif strategy == "all":
        # Retourner toutes les annotations
        return {
            "all_annotations": annotations,
            "label": annotations[-1].get("label") if annotations else None,
            "cue1": annotations[-1].get("cue1") if annotations else None,
            "cue2": annotations[-1].get("cue2") if annotations else None,
            "confidence": np.mean([a.get("confidence", 0) for a in annotations]) if annotations else 0,
            "notes": " | ".join(a.get("notes", "") for a in annotations if a.get("notes"))
        }
    
    return annotations[-1]  # Par défaut: dernière

def has_conflict(annotations: List[Dict]) -> bool:
    """Détecte les conflits entre annotations"""
    ...
    return (len(set(labels)) > 1 or 
            len(set(cues1)) > 1 or 
            len(set(cues2)) > 1)

def get_conflict_details_data(annotations: List[Dict]) -> Dict:
    """Récupère les détails des conflits"""
    labels = [a["label"] for a in annotations if a["label"] is not None]
    cues1 = [a["cue1"] for a in annotations if a["cue1"] is not None]
    cues2 = [a["cue2"] for a in annotations if a["cue2"] is not None]
    ...
    return {
        ...
        "cue1_distribution": dict(Counter(cues1)),
        "cue2_distribution": dict(Counter(cues2)),
        "users": list(set(a["username"] for a in annotations)),
        "annotation_count": len(annotations)
    }

def apply_label_filter(pair: Dict, label_filter: str) -> bool:
    """Applique le filtre de label"""
    if label_filter == "Tous":
        return True
    
    pair_label = pair.get("label")
    
    if label_filter == "Causal (1)" and pair_label == 1:
        return True
    elif label_filter == "Non-causal (0)" and pair_label == 0:
        return True
    elif label_filter == "Incertain (None)" and pair_label is None:
        return True
    elif label_filter == "Non annoté" and "annotation_confidence" not in pair:
        return True
    
    return False

def generate_tabular_export(
    original_data: Dict,
    annotations: List[Annotation],
    merge_strategy: str,
    include_labels: bool,
    ...
    """Génère un export tabulaire"""
    
    rows = []
    
    if "pairs" not in original_data:
        return pd.DataFrame()
    ...
    for pair in original_data["pairs"]:
        ...
        # Préparer la ligne
        row = {
            "dataset": pair.get("narrative_id", "").split('.')[0] if pair.get("narrative_id") else "",
            "pair_id": f"{pair.get('event1_id')}_{pair.get('event2_id')}",
            "event1_id": pair.get("event1_id"),
            "event2_id": pair.get("event2_id"),
            "event1_text": pair.get("event1_text", "")[:200] + "..." if len(pair.get("event1_text", "")) > 200 
                          else pair.get("event1_text", ""),
            "event2_text": pair.get("event2_text", "")[:200] + "..." if len(pair.get("event2_text", "")) > 200 
                          else pair.get("event2_text", ""),
            "original_label": pair.get("label"),
            ...
            # Cues originaux
            "original_event1_has_causal_cue": pair.get("event1_has_causal_cue", False),
            "original_event1_causal_cue_text": pair.get("event1_causal_cue_text", ""),
            "original_event2_has_causal_cue": pair.get("event2_has_causal_cue", False),
            "original_event2_causal_cue_text": pair.get("event2_causal_cue_text", ""),
            
            "has_annotations": key in annotations_by_pair
        }
        
        if key in annotations_by_pair:
            pair_anns = annotations_by_pair[key]
            
            # Convertir en dicts pour la fusion
            ann_dicts = [{
                "username": a.username,
                "cue1": a.cue1,
                "cue2": a.cue2,
            ...
            merged = merge_annotations_data(ann_dicts, merge_strategy)
            
            # Ajouter les annotations fusionnées
            if include_labels and merged.get("label") is not None:
                row["annotated_label"] = merged["label"]
                row["label_source"] = merge_strategy
            
            if include_cues:
                if merged.get("cue1") is not None:
                    row["annotated_cue1"] = merged["cue1"]
                    row["final_event1_has_causal_cue"] = merged["cue1"]
                
                if merged.get("cue2") is not None:
                    row["annotated_cue2"] = merged["cue2"]
                    row["final_event2_has_causal_cue"] = merged["cue2"]
            
            if include_confidence:
                row["annotation_confidence"] = merged.get("confidence", 0)
            
            if include_notes and merged.get("notes"):
                row["annotation_notes"] = merged["notes"]
            
            if include_conflicts and len(pair_anns) > 1:
                row["has_conflict"] = has_conflict(ann_dicts)
                row["annotation_count"] = len(pair_anns)
                row["annotators"] = ", ".join(sorted(set(a.username for a in pair_anns)))
            
            row["last_annotation"] = max(a.annotated_at for a in pair_anns).isoformat()
        
        # Appliquer le filtre
        if not apply_label_filter(row, label_filter):
            continue
    ...
    base_columns = [
        'dataset', 'pair_id', 'event1_id', 'event2_id',
        'event1_text', 'event2_text',
        'original_label', 'annotated_label', 'label_source',
        'original_event1_has_causal_cue', 'annotated_cue1', 'final_event1_has_causal_cue',
        'original_event2_has_causal_cue', 'annotated_cue2', 'final_event2_has_causal_cue',
        'annotation_confidence', 'has_conflict', 'annotators', 'last_annotation'
    ]
...
def download_json_export(data: Dict, dataset_name: str):
    """Téléchargement JSON"""
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"{dataset_name}_export_{timestamp}.json"
    ...
    with st.expander("👁️ Aperçu du JSON"):
        preview = data.copy()
        if "pairs" in preview and len(preview["pairs"]) > 3:
            preview["pairs"] = preview["pairs"][:3]
        st.json(preview)

def download_csv_export(df: pd.DataFrame, dataset_name: str):
    """Téléchargement CSV"""
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"{dataset_name}_export_{timestamp}.csv"
...
def download_excel_export(df: pd.DataFrame, dataset_name: str):
    """Téléchargement Excel"""
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"{dataset_name}_export_{timestamp}.xlsx"
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix='.xlsx') as tmp:
        with pd.ExcelWriter(tmp.name, engine='openpyxl') as writer:
            ...
            # Ajouter un onglet de résumé
            summary_data = create_summary(df)
...
def create_summary(df: pd.DataFrame) -> List[Dict]:
    """Crée un résumé des données"""
    
    summary = []
    
    total_rows = len(df)
    summary.append({"Statistique": "Total des paires", "Valeur": total_rows})
    
    if 'has_annotations' in df.columns:
        annotated = df['has_annotations'].sum()
        summary.append({"Statistique": "Paires annotées", "Valeur": f"{annotated} ({annotated/total_rows*100:.1f}%)"})
    
    if 'annotated_label' in df.columns:
        label_counts = df['annotated_label'].value_counts()
        for label, count in label_counts.items():
            summary.append({"Statistique": f"Label {label}", "Valeur": f"{count} ({count/total_rows*100:.1f}%)"})
    
    if 'has_conflict' in df.columns:
        conflicts = df['has_conflict'].sum()
        summary.append({"Statistique": "Conflits", "Valeur": f"{conflicts} ({conflicts/total_rows*100:.1f}%)"})
    
    return summary

def display_export_statistics(original_data: Dict, annotations: List[Annotation]):
    """Affiche les statistiques de l'export"""
    
    st.divider()
    st.subheader("📊 Statistiques de l'export")
    
    total_pairs = len(original_data.get("pairs", []))
    annotated_pairs = len(set((a.narrative_id, a.event1_id, a.event2_id) for a in annotations))
    
    # Compter les annotations de cues
    cue1_true = sum(1 for a in annotations if a.cue1)
    cue2_true = sum(1 for a in annotations if a.cue2)
    ...
    with col2:
        st.metric("Paires annotées", f"{annotated_pairs} ({annotated_pairs/total_pairs*100:.0f}%)")
    
    with col3:
        st.metric("Cue1 annotés (True)", cue1_true)
    
    with col4:
        st.metric("Cue2 annotés (True)", cue2_true)
    
    # Graphique
    if annotations:
        fig = make_subplots(rows=1, cols=2, subplot_titles=("Distribution des labels", "Cues annotés"))
        ...
        if labels:
            label_counts = Counter(labels)
            fig.add_trace(
                go.Bar(x=list(label_counts.keys()), y=list(label_counts.values()), name="Labels"),
                row=1, col=1
            )
        ...
        cue_data = {
            'Cue1 True': cue1_true,
            'Cue1 False': len(annotations) - cue1_true,
            'Cue2 True': cue2_true,
            'Cue2 False': len(annotations) - cue2_true
        }
        
        fig.add_trace(
            go.Bar(x=list(cue_data.keys()), y=list(cue_data.values()), name="Cues"),
            row=1, col=2
        )
...
def dashboard_page():
    ...
    # Liste des derniers datasets
    st.divider()
    st.subheader("📋 Derniers datasets")
    
    for dataset in datasets[:5]:  # 5 derniers
        with st.container(border=True):
...
def main():
    ...
    with st.sidebar:
        ...
        # Statut de connexion
        st.caption(f"🔗 {'Connecté' if gsheets.connected else 'Mode local'}")
    
    # Page sélectionnée
    if selected_page == "✏️ Annoter":
        annotate_page()
    elif selected_page == "📁 Gérer datasets" and st.session_state.is_admin:
        dataset_management_page()
