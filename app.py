...
def annotate_page():
    ...
    with st.expander("📊 Informations supplémentaires", expanded=False):
        ...
        # Section for admin pair evaluation
        if st.session_state.is_admin:
            st.markdown("---")
            st.markdown("#### 🔍 Évaluation de la paire (Admin)")
            evaluation = st.radio(
                "Cette paire est-elle utile pour l'annotation ?",
                ["✅ Utile", "⚠️ Discutable", "❌ Inutile"],
                horizontal=True,
                key=f"eval_{current_pair.pair_id}"
            )
            
            if evaluation == "❌ Inutile":
                reason = st.selectbox(
                    "Raison",
                    ["Texte ambigu", "Paire incorrecte", "Doublon", "Autre problème"],
                    key=f"reason_{current_pair.pair_id}"
                )
                
                if reason == "Autre problème":
                    details = st.text_area(
                        "Détails",
                        placeholder="Expliquez le problème...",
                        key=f"details_{current_pair.pair_id}"
                    )
                
                st.warning("⚠️ Cette paire peut être supprimée avec le bouton '🗑️ Supprimer' en bas")
    ...
    if st.session_state.show_delete_confirm:
        ...
        if st.session_state.is_admin:
            col1, col2, col3, col4, col5 = st.columns([1, 2, 2, 1, 1])  # 5 columns for admin
        else:
            col1, col2, col3, col4 = st.columns([1, 2, 2, 1])  # 4 columns for regular users
